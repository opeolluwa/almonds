...
const todos = ref<Todo[]>([
  { id: 1, title: "Review pull requests", completed: false, priority: "high", dueDate: "Today" },
  { id: 2, title: "Update documentation", completed: false, priority: "medium", dueDate: "Tomorrow" },
  { id: 3, title: "Fix navigation bug", completed: true, priority: "high", dueDate: "Yesterday" },
  { id: 4, title: "Design new dashboard layout", completed: false, priority: "low", dueDate: "Feb 20" },
  { id: 5, title: "Write unit tests", completed: false, priority: "medium", dueDate: "Feb 19" },
  { id: 6, title: "Optimize database queries", completed: true, priority: "high", dueDate: "Feb 15" },
]);
  ...
  <NuxtLayout name="default">
    <template #main_content>
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">Todo</h1>
        <div class="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-0.5">
          <button
            ...
            :class="filter === f ? 'bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 shadow-sm' : 'text-gray-500 dark:text-gray-400'"
            @click="filter = f"
          >
            {{ f }}
          </button>
      ...
      <div class="flex flex-col gap-2">
        ...
        >
          ...
          <UIcon name="heroicons:flag" class="size-4" :class="priorityColor[todo.priority]" />
    ...
    <template #side_content>
      ...
      <div class="flex flex-col gap-3 mb-6">
        ...
        <div class="bg-emerald-50 dark:bg-emerald-950 rounded-lg p-3">
          ...
          <p class="text-xs text-emerald-500 dark:text-emerald-400">Completed</p>
        </div>
      </div>

      <USeparator class="my-4" />

      <h2 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Priority</h2>
      <div class="flex flex-col gap-2">
        <div class="flex items-center justify-between text-sm">
          ...
          <span class="text-xs text-gray-400">{{ todos.filter((t) => t.priority === "high").length }}</span>
        </div>
        <div class="flex items-center justify-between text-sm">
          <div class="flex items-center gap-2">
          ...
          <span class="text-xs text-gray-400">{{ todos.filter((t) => t.priority === "medium").length }}</span>
        </div>
        <div class="flex items-center justify-between text-sm">
          <div class="flex items-center gap-2">
          ...
          <span class="text-xs text-gray-400">{{ todos.filter((t) => t.priority === "low").length }}</span>
        </div>
      </div>
    </template>
  </NuxtLayout>
