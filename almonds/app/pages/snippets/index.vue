...
const snippets = [
  { title: "Vue Composable Template", language: "TypeScript", lines: 24, date: "Feb 16, 2026", preview: "export function useCounter() {\n  const count = ref(0)\n  return { count }\n}" },
  { title: "Tailwind Card Component", language: "Vue", lines: 18, date: "Feb 15, 2026", preview: '<div class="rounded-lg p-4 bg-white shadow">\n  <slot />\n</div>' },
  { title: "Fetch API Wrapper", language: "TypeScript", lines: 32, date: "Feb 14, 2026", preview: "async function fetchAPI<T>(url: string): Promise<T> {\n  const res = await fetch(url)\n  return res.json()\n}" },
  { title: "CSS Grid Layout", language: "CSS", lines: 12, date: "Feb 12, 2026", preview: ".grid-layout {\n  display: grid;\n  grid-template-columns: repeat(3, 1fr);\n}" },
];
...
<template>
  <NuxtLayout name="default">
        ...
        >
          <div class="p-4">
            <div class="flex items-center justify-between mb-2">
              <h3 class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ snippet.title }}</h3>
              <div class="flex items-center gap-3">
                <span class="px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-xs text-gray-500 dark:text-gray-400">{{ snippet.language }}</span>
                <span class="text-xs text-gray-400">{{ snippet.lines }} lines</span>
              </div>
            </div>
            <pre class="bg-gray-900 text-gray-100 rounded-md p-3 text-xs overflow-x-auto"><code>{{ snippet.preview }}</code></pre>
          </div>
    ...
    <template #side_content>
      <h2 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Languages</h2>
      <div class="flex flex-col gap-1">
        ...
        >
          <span>{{ lang }}</span>
          <span class="text-xs text-gray-400">{{ lang === 'All' ? snippets.length : snippets.filter(s => s.language === lang).length }}</span>
        </div>
      </div>

      <USeparator class="my-4" />

      <h2 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Recent</h2>
      <div class="flex flex-col gap-2">
        ...
        >
          <p class="font-medium text-gray-700 dark:text-gray-300 truncate text-xs">{{ snippet.title }}</p>
          <p class="text-xs text-gray-400">{{ snippet.date }}</p>
        </div>
      </div>
    </template>
