  ...
  <NuxtLayout name="default">
    <template #main_content>
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">Scratch Pad</h1>
    ...
    <template #side_content>
      <h2 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Pads</h2>
      <div class="flex flex-col gap-1 mb-6">
        <div
          v-for="pad in pads"
          :key="pad.title"
          class="flex items-center gap-3 py-2.5 px-3 rounded-lg cursor-pointer transition-colors"
          :class="pad.active ? 'bg-violet-50 dark:bg-violet-950 text-violet-700 dark:text-violet-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'"
        >
          <UIcon name="heroicons:document" class="size-4" />
          <div class="flex-1">
            <p class="text-sm font-medium">{{ pad.title }}</p>
            <p class="text-xs" :class="pad.active ? 'text-violet-400' : 'text-gray-400'">{{ pad.date }}</p>
          </div>
        </div>
      </div>
      ...
      <h2 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Quick Actions</h2>
      <div class="flex flex-col gap-2">
        <button class="flex items-center gap-2 py-2 px-3 rounded-lg text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors w-full text-left">
        ...
        <button class="flex items-center gap-2 py-2 px-3 rounded-lg text-sm text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950 transition-colors w-full text-left">
